#!/usr/bin/env python3
"""C4 modelV2 road geometry helpers for G80 radar monitor.
...
def _sample_xy(xs: Iterable, ys: Iterable, max_points: int = MAX_MODEL_POINTS) -> list[dict]:
  ...
  for i in range(0, n, step):
    x = _finite(xs[i])
    # Convert model convention to radar/display convention: +left.
    y = -_finite(ys[i])
...
def _path_y(path: list[dict], x: float) -> float | None:
  if not path:
    return None
  if len(path) == 1:
...
def path_y_at_x(road_model: dict | None, x: float) -> float | None:
  if not road_model:
    return None
  return _path_y(road_model.get('path', []), float(x))
...
def project_to_path(x: float, y: float, road_model: dict | None) -> dict | None:
  ...
  for i in range(len(path) - 1):
    ...
    ex, ey = px - qx, py - qy
    dist2 = ex * ex + ey * ey
    # cross(tangent, point-projection): positive means point lies left of path.
    cross = dx * ey - dy * ex
...
def annotate_object(o: dict, road_model: dict | None) -> dict:
  ...
  projection = project_to_path(x, y, road_model) if x >= -0.5 else None
