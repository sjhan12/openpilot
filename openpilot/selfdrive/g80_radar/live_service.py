...
latest_state={'version':12,'objects':[],'corner_fused_objects':[],'front_objects':[],'all_fused_objects':[],'filtered_objects':[],'raw_objects':[]}
...
</style></head><body><div class="top"><div class="tabs"><button data-mode="corner" class="active">CORNER FUSED</button><button data-mode="front">FRONT</button><button data-mode="all">ALL FUSED</button><button data-mode="filtered">FILTERED</button><button data-mode="raw">RAW</button></div><div class="range"><button data-range="short" class="active">SHORT 1:1</button><button data-range="long">LONG 1:1</button><button data-range="wide">WIDE</button></div><div class="spacer"></div><div class="brand"><b>G80 5-Radar (v12)</b><br><em>Teacher Fusion · Visible/Auto-Bus</em></div><div class="live">● LIVE 10 Hz</div><button id="fs">전체</button></div><div class="main"><div class="scene"><canvas id="radar"></canvas></div><div class="side"><div class="card"><h3>상태</h3><div id="summary"></div></div><div class="card"><h3>차선 점유</h3><div id="zones" class="zones"></div></div><div class="card"><h3>Teacher</h3><div id="teachers"></div></div><div id="objects" class="card objList"></div><div class="footer">Browser UI is on-demand: no /state polling → no verbose UI JSON/render-state work on C4. Android UDP 28991 stays live.</div></div></div><script>
...
def main():
  ...
  tracks=TrackStore(ttl_s=.75,continuity_s=.30);corner_fuser=CornerFusionTracker();scc_matcher=SccFrontTeacherMatcher();rear_teacher=[];rear_teacher_by_bus={};scc_teacher=None;scc_teacher_by_bus={};SCC_BUS=int(os.getenv('G80_SCC_BUS',str(DEFAULT_SCC_BUS)))
  ...
  while True:
    ...
    if now>=next_pub:
      ...
      core={'version':12,'mono_ns':now_ns,'objects':all_fused,'all_fused_objects':all_fused,
      ...
      if ui_active:
        ...
        out.update({
          ...
          'browser_active':True,
          'notes':{
            'ui':'render-lite-on-demand-teacher-visible',
