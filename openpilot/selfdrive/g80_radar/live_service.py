...
from openpilot.selfdrive.g80_radar.shadow_leads import extract_model_path,ShadowLeadVerifier,snapshot_production_radar_state
...
def main():
  ...
  tracks=TrackStore(ttl_s=.75,continuity_s=.30);corner_fuser=CornerFusionTracker();corner_vehicle_tracker=VehicleFootprintTracker('VC');scc_matcher=SccFrontTeacherMatcher();camera_fuser=CameraRadarFusion();shadow_verifier=ShadowLeadVerifier();shadow_logger=ShadowLogger();front_preview=StandardFrontPreview();camera_leads=[];model_path=[];model_path_recv_ns=0;v_ego=0.0;v_ego_recv_ns=0;production=None;rear_teacher=[];rear_teacher_by_bus={};scc_teacher=None;scc_teacher_by_bus={};SCC_BUS=int(os.getenv('G80_SCC_BUS',str(DEFAULT_SCC_BUS)))
  ...
  while True:
    ...
    if mmsg is not None:
      ...
      model_path=extract_model_path(mmsg.modelV2)
    ...
    if now>=next_pub:
      ...
      core={'version':19,'mono_ns':now_ns,'objects':sensor_fused,
            'sensor_fused_objects':sensor_fused,'all_fused_objects':sensor_fused,
            'radar_fused_objects':radar_all_fused,
            'corner_fused_objects':corner_vehicle,'corner_radar_objects':corners,'front_objects':fronts,'front_sensor_objects':front_sensor,
            'standard_front_preview':stdp['points'],'standard_front_preview_stats':stdp['stats'],
      ...
      if ui_active or debug_due:
        ...
        out.update({
          ...
          'notes':{
            'ui':'v19-monitor-only-front-preview-plus-360',
            'display_scale':'SHORT/LONG 1:1 + WIDE/DRIVE visual',
        ...
        compact['filtered_objects']=filt
        compact['raw_objects']=raw
