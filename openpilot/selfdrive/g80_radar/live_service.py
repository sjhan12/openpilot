...
latest_state={'version':20,'objects':[],'corner_fused_objects':[],'front_objects':[],'standard_front_preview':[],'all_fused_objects':[],'filtered_objects':[],'raw_objects':[]}

HTML=r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover"><title>G80 5-Radar V20 Monitor</title><style>
...
</style></head><body><div class="top"><div class="tabs"><button data-mode="corner" class="active">CORNER FUSED</button><button data-mode="front">FRONT</button><button data-mode="stdpreview">STD PREVIEW</button><button data-mode="all">ALL FUSED</button><button data-mode="filtered">FILTERED</button><button data-mode="raw">RAW</button><button data-mode="shadow">SHADOW L1/L2</button></div><div class="range"><button data-range="short" class="active">SHORT 1:1</button><button data-range="long">LONG 1:1</button><button data-range="wide">WIDE</button><button data-range="drive">DRIVE 3-LANE</button></div><div class="spacer"></div><div class="brand"><b>G80 5-Radar (v20)</b><br><em>MONITOR ONLY · C4 Curved Road</em></div><div class="live">● LIVE 10 Hz</div><button id="fs">전체</button></div><div class="main"><div class="scene"><canvas id="radar"></canvas></div><div class="side"><div class="card"><h3>상태</h3><div id="summary"></div></div><div class="card"><h3>차선 점유</h3><div id="zones" class="zones"></div></div><div class="card"><h3>Teacher</h3><div id="teachers"></div></div><div id="objects" class="card objList"></div><div class="footer">v20 MONITOR ONLY: C4 modelV2 road curvature + path-relative lane classification. No radarTracks/radarState publish, no CAN TX.</div></div></div><script>
...
def main():
  ...
  while True:
    ...
    if now>=next_pub:
      ...
      core={'version':20,'mono_ns':now_ns,'objects':sensor_fused_view,
      ...
      if ui_active or debug_due:
        ...
        out.update({
          ...
          'notes':{
            'ui':'v20-monitor-only-c4-curved-road-plus-front-preview-360',
            'display_scale':'SHORT/LONG 1:1 + WIDE/DRIVE visual; C4 modelV2 curved road overlay',
            ...
            'road_geometry':'C4 modelV2 position/laneLines; display + Frenet-like lane classification only',
