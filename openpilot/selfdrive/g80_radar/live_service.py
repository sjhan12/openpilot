...
</style></head><body><div class="top"><div class="tabs"><button data-mode="corner" class="active">CORNER FUSED</button><button data-mode="front">FRONT</button><button data-mode="all">ALL FUSED</button><button data-mode="filtered">FILTERED</button><button data-mode="raw">RAW</button><button data-mode="shadow">SHADOW L1/L2</button></div><div class="range"><button data-range="short" class="active">SHORT 1:1</button><button data-range="long">LONG 1:1</button><button data-range="wide">WIDE</button><button data-range="drive">DRIVE 3-LANE</button></div><div class="spacer"></div><div class="brand"><b>G80 5-Radar (v17)</b><br><em>Shadow dPath Guarded · Persistent Logger</em></div><div class="live">● LIVE 10 Hz</div><button id="fs">전체</button></div><div class="main"><div class="scene"><canvas id="radar"></canvas></div><div class="side"><div class="card"><h3>상태</h3><div id="summary"></div></div><div class="card"><h3>차선 점유</h3><div id="zones" class="zones"></div></div><div class="card"><h3>Teacher</h3><div id="teachers"></div></div><div id="objects" class="card objList"></div><div class="footer">v17: persistent 5 Hz shadow logger under /data/radar + event-trigger records. Logs survive git updates.</div></div></div><script>
...
def main():
  ...
  tracks=TrackStore(ttl_s=.75,continuity_s=.30);corner_fuser=CornerFusionTracker();scc_matcher=SccFrontTeacherMatcher();camera_fuser=CameraRadarFusion();shadow_verifier=ShadowLeadVerifier();shadow_logger=ShadowLogger();camera_leads=[];model_path=[];model_path_recv_ns=0;v_ego=0.0;v_ego_recv_ns=0;production=None;rear_teacher=[];rear_teacher_by_bus={};scc_teacher=None;scc_teacher_by_bus={};SCC_BUS=int(os.getenv('G80_SCC_BUS',str(DEFAULT_SCC_BUS)))
  ...
  while True:
    ...
    if now>=next_pub:
      ...
      core={'version':17,'mono_ns':now_ns,'objects':sensor_fused,
            ...
            'camera_fusion_stats':camf['stats'],'shadow_leads':shadow,
