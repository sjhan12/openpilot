...
from openpilot.selfdrive.g80_radar.teacher_fusion import SCC_CONTROL_ADDR,DEFAULT_SCC_BUS,decode_scc_teacher,SccFrontTeacherMatcher
...
latest_state={'version':11,'objects':[],'corner_fused_objects':[],'front_objects':[],'all_fused_objects':[],'filtered_objects':[],'raw_objects':[]}
...
def main():
  ...
  tracks=TrackStore(ttl_s=.75,continuity_s=.30);corner_fuser=CornerFusionTracker();scc_matcher=SccFrontTeacherMatcher();rear_teacher=[];scc_teacher=None;SCC_BUS=int(os.getenv('G80_SCC_BUS',str(DEFAULT_SCC_BUS)))
  udp=socket.socket(socket.AF_INET,socket.SOCK_DGRAM);udp.setsockopt(socket.SOL_SOCKET,socket.SO_BROADCAST,1)
  diag={'corner_A_frames':0,'corner_B_frames':0,'corner_decoded_total':0,'corner_rear_total':0,'corner_front_total':0,'corner_A_by_bus':{},'corner_B_by_bus':{},'scc_frames_by_bus':{},'scc_teacher_updates':0,'last_transport_lag_ms':None}
      ...
      for f in msg.can:
        ...
        if addr==SCC_CONTROL_ADDR:
          ...
          if bus==SCC_BUS:
            ...
            if st is not None: scc_teacher=st;diag['scc_teacher_updates']+=1
        if addr in CORNER_A:
        ...
        elif addr==0x1EA and bus==1: rear_teacher=decode_rear_teacher_1ea(dat)
      processed+=1
    now=time.monotonic()
    if now>=next_pub:
      now_ns=time.monotonic_ns();raw=tracks.snapshot(now_ns);filt=filtered_objects(raw,rear_teacher);cf=corner_fuser.update(filt,now_ns);fronts=build_front_objects(filt);fronts,scc_match_status=scc_matcher.update(fronts,scc_teacher,now_ns);combined=associate_corner_front(cf['corner_fused_objects'],fronts);corners=combined['corner_objects'];fronts=combined['front_objects'];all_fused=combined['all_fused_objects'];assocs=combined['associations'];zones=occupied_zones(corners)
      ...
      core={'version':11,'mono_ns':now_ns,'objects':all_fused,'all_fused_objects':all_fused,
            'corner_fused_objects':corners,'front_objects':fronts,
