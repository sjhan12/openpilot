...
from openpilot.selfdrive.g80_radar.kalman_motion import KalmanMotionTracker
...
latest_state={'version':22,'objects':[],'corner_fused_objects':[],'front_objects':[],'standard_front_preview':[],'all_fused_objects':[],'filtered_objects':[],'raw_objects':[]}

HTML=r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover"><title>G80 5-Radar V22 Kalman Monitor</title><style>
...
</style></head><body><div class="top"><div class="tabs"><button data-mode="corner" class="active">CORNER FUSED</button><button data-mode="front">FRONT</button><button data-mode="stdpreview">STD PREVIEW</button><button data-mode="all">ALL FUSED</button><button data-mode="filtered">FILTERED</button><button data-mode="raw">RAW</button><button data-mode="shadow">SHADOW L1/L2</button></div><div class="range"><button data-range="short" class="active">SHORT 1:1</button><button data-range="long">LONG 1:1</button><button data-range="wide">WIDE</button><button data-range="drive">DRIVE 3-LANE</button></div><div class="spacer"></div><div class="brand"><b>G80 5-Radar (v22)</b><br><em>MONITOR ONLY · Kalman Trajectory</em></div><div class="live">● LIVE 10 Hz</div><button id="fs">전체</button></div><div class="main"><div class="scene"><canvas id="radar"></canvas></div><div class="side"><div class="card"><h3>상태</h3><div id="summary"></div></div><div class="card"><h3>차선 점유</h3><div id="zones" class="zones"></div></div><div class="card"><h3>Teacher</h3><div id="teachers"></div></div><div id="objects" class="card objList"></div><div class="footer">v22 MONITOR ONLY: C4 curved-road + 6-state CA Kalman trajectory + corner-sector colors. No radarTracks/radarState publish, no CAN TX.</div></div></div><script>
...
def main():
  ...
  tracks=TrackStore(ttl_s=.75,continuity_s=.30);corner_fuser=CornerFusionTracker();corner_vehicle_tracker=VehicleFootprintTracker('VC');scc_matcher=SccFrontTeacherMatcher();camera_fuser=CameraRadarFusion();shadow_verifier=ShadowLeadVerifier();shadow_logger=ShadowLogger();front_preview=StandardFrontPreview();motion_tracker=KalmanMotionTracker('KF');corner_motion_tracker=KalmanMotionTracker('KFC');front_motion_tracker=KalmanMotionTracker('KFF');camera_leads=[];model_path=[];road_model={};model_path_recv_ns=0;v_ego=0.0;v_ego_recv_ns=0;production=None;rear_teacher=[];rear_teacher_by_bus={};scc_teacher=None;scc_teacher_by_bus={};SCC_BUS=int(os.getenv('G80_SCC_BUS',str(DEFAULT_SCC_BUS)))
  ...
  while True:
    ...
    if now>=next_pub:
      ...
      raw_view=annotate_objects(raw,road_view);filt_view=annotate_objects(filt,road_view);corner_vehicle_road=annotate_objects(corner_vehicle,road_view);corners_view=annotate_objects(corners,road_view);fronts_view=annotate_objects(fronts,road_view);radar_all_view=annotate_objects(radar_all_fused,road_view);sensor_fused_road=annotate_objects(sensor_fused,road_view);front_sensor_road=annotate_objects(front_sensor,road_view);std_points_view=annotate_objects(stdp['points'],road_view);camera_leads_view=annotate_objects(camera_leads,road_view)
      sensor_fused_view,kalman_stats=motion_tracker.update(sensor_fused_road,road_view,now_ns)
      corner_vehicle_view,corner_kalman_stats=corner_motion_tracker.update(corner_vehicle_road,road_view,now_ns)
      front_sensor_view,front_kalman_stats=front_motion_tracker.update(front_sensor_road,road_view,now_ns)
      ...
      core={'version':22,'mono_ns':now_ns,'objects':sensor_fused_view,
            ...
            'kalman_motion_stats':kalman_stats,'corner_kalman_motion_stats':corner_kalman_stats,'front_kalman_motion_stats':front_kalman_stats,
      ...
      if ui_active or debug_due:
        ...
        out.update({
          ...
          'notes':{
            'ui':'v22-monitor-only-kalman-trajectory-perspective-road-360',
            ...
            'motion_prediction':'per-vehicle 6-state constant-acceleration Kalman; Cartesian + Frenet; 0.5/1/2/3s preview only',
